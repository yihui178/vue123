  ...
  <el-card style="margin: 10px; padding: 10px 10px 0 10px; height: 80vh">
    <el-row>
      <el-button
        ...
        v-if="this.userInfo.level === 0 || this.userInfo.level === 1"
        >筛选</el-button
      >
      <el-button
        @click="edit"
      ...
      <el-button
        ...
        @click="click"
        v-if="this.userInfo.level === 0"
        :disabled="form.grade === '' || form.year === '' || form.term === ''"
        >录入</el-button
      >
      <el-button
        type="danger"
...
export default {
  ...
  methods: {
    ...
    getTimetableByStudent() {
      ...
      this.axiosHelper
        .get("/api/sms/timetable/getTimetableByStudent", { params: obj })
        .then((response) => {
          let data = response.data;
          if (data.length > 0) {
            for (let i = 0; i < this.section.length; i++) {
    ...
    getTimetableByTeacher() {
      ...
      this.axiosHelper
        .get("/api/sms/timetable/getTimetableByTeacher", { params: obj })
        .then((response) => {
          let data = response.data;
          if (data.length > 0) {
            for (let i = 0; i < this.section.length; i++) {
